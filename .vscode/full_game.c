...
void PickupItem(Inventory* inv, Room* currentRoom, const char* itemName) {
    ...
    for (int i = 0; i < currentRoom->itemCount; i++) {
        if (strcmp(currentRoom->items[i]->name, itemName) == 0) {
            itemIndex = i;
            break;
        }
    }
    ...
    if (strcmp(itemName, "Rusty Cog") == 0 && inv->capacity == 1) {
...
bool CombineItems(Inventory* inv, const char* item1, const char* item2) {
    ...
    // Find both items
    for (int i = 0; i < inv->count; i++) {
        if (strcmp(inv->items[i].name, item1) == 0) {
            index1 = i;
        }
        if (strcmp(inv->items[i].name, item2) == 0) {
            index2 = i;
        }
    }
    ...
    if (inv->items[index1].isCombinable && 
        (strcmp(inv->items[index1].combineWith, inv->items[index2].name) == 0)) {
        ...
        return true;
               ...
               (strcmp(inv->items[index2].combineWith, inv->items[index1].name) == 0)) {
...
// Interact with objects in rooms
void InteractWith(Room* currentRoom, Inventory* inv, const char* objectName) {
    // Find the object in the room
    for (int i = 0; i < currentRoom->interactableCount; i++) {
        if (strcmp(currentRoom->interactables[i]->name, objectName) == 0) {
            printf("You interact with %s.\n", objectName);
            
            // Specific interaction logic
            if (strcmp(objectName, "Jaguar") == 0 && !currentRoom->interactables[i]->interacted) {
                ...
                // Check if jaguar riddle was solved
                for (int j = 0; j < currentRoom->interactableCount; j++) {
                ...
                if (jaguarSatisfied) {
                    ...
                    if (inv->count < inv->capacity) {
                        strcpy(inv->items[inv->count].name, keyPart->name);
                        strcpy(inv->items[inv->count].description, keyPart->description);
                        inv->items[inv->count].quantity = keyPart->quantity;
                        inv->items[inv->count].isCombinable = keyPart->isCombinable;
                        strcpy(inv->items[inv->count].combineWith, keyPart->combineWith);
                        strcpy(inv->items[inv->count].resultItem, keyPart->resultItem);
                        inv->count++;
                ...
                printf("You examine the tree more closely...\n");
                ...
                if (inv->count < inv->capacity) {
                    strcpy(inv->items[inv->count].name, keycard->name);
                    strcpy(inv->items[inv->count].description, keycard->description);
                    inv->items[inv->count].quantity = keycard->quantity;
                    inv->items[inv->count].isCombinable = keycard->isCombinable;
                    inv->count++;
                ...
                return;
            } else if (strcmp(objectName, "Machine") == 0) {
                if (HasItem(inv, "Clean Cog")) {
                    printf("You insert the Clean Cog into the machine.\n");
                    printf("The machine whirs to life and produces the second part of the golden key!\n");
                    
                    RemoveItemFromInventory(inv, "Clean Cog");
                    
                    Item* keyPart = CreateItem("Key Part 2", 1, 
                                               "The second part of a three-part golden key.", 
                                               true, "Combined Key Parts", "Complete Key");
                    ...
                    printf("The machine seems to be missing a key component. Maybe a cog?\n");
                }
                return;
            } else if (strcmp(objectName, "Crate") == 0) {
                if (HasItem(inv, "Sledgehammer")) {
                    printf("You smash the crate open with the sledgehammer!\n");
                    printf("Inside, you find the third part of the golden key.\n");
                    
                    Item* keyPart = CreateItem("Key Part 3", 1, 
                                               "The third part of a three-part golden key.", 
                                               true, "Complete Key", "Golden Key");
                ...
                } else if (HasItem(inv, "Crowbar")) {
                    printf("You pry the crate open with the crowbar!\n");
                    printf("Inside, you find the third part of the golden key.\n");
                    ...
                    printf("The crate is sturdy. You need a tool to open it.\n");
                }
                return;
            } else if (strcmp(objectName, "Kitchen") == 0) {
                printf("You examine the hi-tech kitchen. There's a blender that looks functional.\n");
                if (HasItem(inv, "Suspicious fruit")) {
                    printf("Would you like to blend the suspicious fruit? (yes/no): ");
                    char answer[10];
                    scanf("%s", answer);
                    
                    if (strcmp(answer, "yes") == 0) {
                        printf("You blend the suspicious fruit into a glowing liquid.\n");
                        RemoveItemFromInventory(inv, "Suspicious fruit");
                        
                        Item* solution = CreateItem("Anti-Rust Solution", 1, 
                                                   "A chemical solution that can clean rust off metal.", 
                                                   true, "Rusty Cog", "Clean Cog");
                        
                        if (inv->count < inv->capacity) {
                            strcpy(inv->items[inv->count].name, solution->name);
                            strcpy(inv->items[inv->count].description, solution->description);
                            inv->items[inv->count].quantity = solution->quantity;
                            inv->items[inv->count].isCombinable = solution->isCombinable;
                            strcpy(inv->items[inv->count].combineWith, solution->combineWith);
                            strcpy(inv->items[inv->count].resultItem, solution->resultItem);
                            inv->count++;
                            
                            printf("You created %s!\n", solution->name);
                            free(solution);
                        } else {
                            printf("Your inventory is full! You can't take the solution.\n");
                            AddItemToRoom(currentRoom, solution);
                        }
                    }
                } else {
                    printf("You might be able to blend something here if you had the right ingredients.\n");
                }
                return;
            }
...
void ProcessCommand(char* command, Inventory* inv, Room** currentRoom, bool* gameRunning, bool* hasWon, FILE* logFile) {
    ...
    else if (strcmp(cmd, "pick") == 0 && strcmp(param1, "up") == 0 && params == 3) {
        PickupItem(inv, *currentRoom, param2);
        sprintf(result, "Attempted to pick up %s", param2);
    }
    else if (strcmp(cmd, "take") == 0 && params >= 2) {
        PickupItem(inv, *currentRoom, param1);
        sprintf(result, "Attempted to take %s", param1);
    }
...
int main() {
    ...
    while (gameRunning) {
        ...
        if (strcmp(currentRoom->name, "Gold Room") == 0) {
    ...
    // Free interactables in each room
